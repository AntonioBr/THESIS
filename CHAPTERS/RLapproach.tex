...
Adopting the linear approach the optimization process is slower but less accurate. This depends on the fact that the movement is as the crow flies. Also using the linear approach the concept of function can be abstracted. The main difference between the linear approach and the parametric approach is that, in the first case, the slope of the function is not dynamically considered.
...
In the next few chapters, starting from studies on $2d$-function learning and adopting the previously described mathematical tools, an experiment about $3d$-function learning will be presented. After that, human learning techniques' performances will be compared to RL agent's ones will be compared.